...
__builtins__.__dict__["_"] = createGetText("jbrout",
                                           os.path.join(
...
from jbrout.common import cd2rd, cd2d, runWith, openURL, \
    dnd_args_to_dir_list, xpathquoter  # for selecteur
from jbrout.commongtk import AlbumCommenter, InputBox, \
    MessageBox, InputQuestion, WinKeyTag, colorToString, Buffer
...
class ListView(ThumbnailsView):
    ...
    def on_drag_data_get_data(self, listview, context, selection, target_id,
                              etime):
    ...
    def on_drag_data_received_data(self, widget, object,
                                   x, y, sdata, code, time):
        """ event drop notified """
    ...
    def notify_selection_change(self, old):
        ...
        if nbSelected == 1:
            try:
                ...
                info_text = "%d / %d" % (focusedItem + 1, len(self.items)) + \
                    " - " + photo_node.name + " - " + \
                    photo_node.resolution + " - " + \
                    cd2d(photo_node.date).strftime('%d %B %Y %H:%M') + \
                    " - [%s/5]" % photo_node.rating + " - " + \
                    ", ".join(photo_node.tags)
            except:
                info_text = ""
        elif nbSelected > 0:
    ...
    def refresh(self):
        ...
        # redraw bottom line (in case of tagguing)
        self.notify_selection_change(self.selection.real_selection)

        #~ self.update_layout()
        self.invalidate_view()

    # constant to optimize display
    rating_stars = [i * '*' + (5 - i) * '-' for i in range(0, 6)]
    ...
    def get_text(self, idx):               # override !
        ...
        a = [", ".join(node.tags),
             ...
             cd2rd(node.date),
             node.name,
             '[' + self.rating_stars[node.rating] + ']'][self.select]
...
#========================================================
    ...
    def fill(self, node, attach):
        ...
        if self.__filter is not None:
            ...
            if self.iter_children(new) is None:
    ...
    def getSelected(self):
        """ return a list of tuple (type_of_check, name, sons_or_self)
             ...
             - sons_or_self : if it's a tag, it is its name, else (a catg),
               it's a list of tags descendant
        """
...
class Window(GladeApp):
    ...
    def on_window_key_press_event(self, widget, b, *args):
        ...
        if isCtrl:
            ...
            #TODO: fix missing keys, they should work only for galley view
...
PIL: %s""" % (sys.version_info[:3] + gtk.pygtk_version + gtk.gtk_version + (Image.VERSION, )))
    ...
    def on_btn_search_clicked(self, widget, *args):
        ...
        if ops:
            libl, xpath = u" and ".join(tops), \
                u"//photo[%s]" % u" and ".join(ops)
        else:
...
def main(canModify=True):
    ...
    #~ if locked:
        #~ if InputQuestion(None,
                #~ _("jBrout appears to already be running are
                #~    you sure you wish to run another copy"),
