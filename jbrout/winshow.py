...
class WinShow(GladeApp):
    ...
    def on_selecteur_menu_select_plugin(self, ib, callback):
        ...
        if self.isModify and not currentNode.isReadOnly:
            ...
            # TODO: if plugin "redate" is called,
            ...
            if (key == "page_up") or (key == "up") or (key == "left"):
                ...
                if self.isModify and not currentNode.isReadOnly:
                    if b.keyval < 255 and b.string.strip() != "":
                        wk = WinKeyTag(_("Apply to this photo"),
                                       b.string, JBrout.tags.getAllTags())
                        ret = wk.loop()
                        self.main_widget.fullscreen()
                        if ret:
                            tag = ret[0]
...
class ImageShow(gtk.DrawingArea):
    ...
    def draw(self, context):
        ...
        if self.display:
            ...
            #        pango.FontDescription("courier 8"))
...
def fit(orig_width, orig_height, dest_width, dest_height,
        zoom=False, pointer_position=(0, 0, 0, 0)):
...
def render(pb, maxw, maxh, zoom=1, pointer_position=(0, 0, 0, 0)):
    ...
    if pointer_position == (0, 0, 0, 0) or not zoom:
        ratioy = 1.0 / 2
        ratiox = ratioy
    else:
        ...
        if mouse_x > image_x:
            mouse_x = image_x
        # remove eventual space
        mouse_y = max(mouse_y - (screen_height - maxh), 0)
...
#        self.quit(True,
#            self.tbufComment.gset_mnemonic_modifieret_text(start,
#                     end, False))
#
